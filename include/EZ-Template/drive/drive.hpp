...
class Drive{
 public:
  ...
  /**
   *Function to go to a coordinate relative to the bot
  */
  void go_to(double x, double y, int drive_speed, int turn_speed);
  void abs_go_to(double x, double y, int drive_speed, int turn_speed);
  void pure_pursuit(float waypoints[][2], int waypointCount, float lookahead, float drive_speed, bool cont);
...
#define DRIVE_ROTATION 3
  ...
  public:void get_odometry();
  public:void set_odometry(double x, double y, double angle);
  double prevLeft;
  double prevRight;
  ...
  void recalibrate_odometry();//resets veleocity to 0
  ...
  void aimbot_angle();
  void aimbot_angle_and_shoot(void (*shoot)());
};
